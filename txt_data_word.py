...
# n o i t w a s n ' t b l a c k m o n d a y ||| 2 2 3 3 5 6 
	...
	def __init__(self, train_fname=None, test_fname=None, dict_fname=None, dataset_fnames=[], gen_dict=False, only_dict=True, DICT_SIZE=10000, raw_file=False):	
                if not os.path.exists(dict_fname+'.npz'):
                    only_dict = False
                    gen_dict = True
		    ...
		    self.c_dim = 1
                ...
                if gen_dict:
                    ...
                    self.Cdict, self.num2c, self.c_dim = self.reduce_dict(self.Ccnt, DICT_SIZE=256)
                ...
                self.idx_spliter = [50, 100, 150, 200, 250, 300, 400, 500, 600] #split the sentence length in group to speed up, the 50 contains all sentences which length smaller than 50, the batch length will be 50
		self.MAX_TIME = max(self.idx_spliter) #one sentences length no more than 300 characters 
		...
		char = char[idx]
                ...
                for b in xrange(batch_size):
                    batch_cnt += len(segs[b])-1
                    for i in segs[b]:
                        char_max_len = max(char_max_len, i)
                tchar = NP.zeros((batch_cnt, char_max_len))-1
                batch_cnt = 0
                for b in xrange(batch_size):
                    pos = 0
                    for i in xrange(len(segs[b])-1):
                        tchar[batch_cnt][:segs[b][i]] = char[b][pos:pos+segs[b][i]]
                        word_idx1[b][i] = batch_cnt
                        word_idx2[b][i] = segs[b][i]-1
                        batch_cnt += 1
                        pos += segs[b][i]
		...
		return char, char_label, word_idx1, word_idx2, word
